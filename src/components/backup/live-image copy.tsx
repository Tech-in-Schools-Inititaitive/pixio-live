...
import { PromptContext } from '../PromptContext'; // Adjust the path as needed
...
export class LiveImageShapeUtil extends FrameShapeUtil {
  ...
  override component(shape: TLFrameShape) {
    const { promptText, setPromptText } = useContext(PromptContext); // Include setPromptText here
    ...
    const sendCurrentData = useMemo(() => {
      return debounce((message: string) => {
        ...
        const updatedMessage = JSON.stringify({
          ...messageObject,
          prompt: promptText, // Use promptText from context here
        });
    
        // Log the message details except for the image_url
        console.log('Sending message with details:');
        console.log(`Prompt: ${promptText}`);
        console.log(`Seed: ${messageObject.seed}`);
        console.log(`Sync Mode: ${messageObject.sync_mode}`);
        console.log(`Strength: ${messageObject.strength}`);
        console.log(`Guidance Scale: ${messageObject.guidance_scale}`);
